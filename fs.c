...
namex(char *path, int nameiparent, char *name)
{
	...
	// Don't let the process access root if it's associated with a container
	if ((myproc() != NULL) && (myproc()->fakeroot == 1))
	{
		if (*path == '/')
		{
			ip = iget(myproc()->cwd->dev, myproc()->cwd->inum);
		}
		else
		{
			ip = idup(myproc()->cwd);
	...
	while ((path = skipelem(path, name)) != 0) {
		...
		// Return root of container if associated with a container
		if (myproc()->fakeroot == 1)
		{
			if ((ip == iget(myproc()->cwd->dev, myproc()->cwd->inum)) &&  (strncmp(name, "..", 5) == 0))
				return ip;
		}
