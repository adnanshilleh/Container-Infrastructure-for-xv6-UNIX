...
fork(void)
{
	...
	*np->tf    = *curproc->tf;
	...
	if ((myproc() != NULL) && (np != NULL) && (curproc->fakeroot != NULL))
	{
		//np->fakeroot;
		np->frootpath = curproc->parent->frootpath;
	}
...
// Blocking for condition variable seperate from mutex
// to prevent breaking mutual exclusion
void
cv_wait(int muxid)
{
	...
	if (currproc == 0) panic("SLEEP");

	acquire(&ptable.lock);
	release(&mutex_table.max_muxarr[muxid].lock);

	//Specify the muxtex id that cv is associated with
	currproc->condvar_blocked = muxid;
	currproc->state = SLEEPING;

	sched();

	currproc->condvar_blocked = -1;

	//Release mutex and block waiting on condition variable
	release(&ptable.lock);
	acquire(&mutex_table.max_muxarr[muxid].lock);
}
...
cv_signal(int muxid)
{
	...
	//Check if any threads sleeping that are blocked on condition variable
	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
		if (p->state == SLEEPING && p->condvar_blocked == muxid)
		{
			//Wake up thread blocked on cv
			p->state = RUNNABLE;
			enqueue(p);
		}
...
futex(struct mutex* m)
{
	while (m->thread != -1)
	{
		...
		currproc->condvar_blocked = m->index;
		currproc->state = SLEEPING;
...
{
	//Check that priority is within min or max priority allowed
	if (priority < PRIO_MIN || priority > PRIO_MAX) return -1;
	...
	//If process tries to increase priority, return an error
