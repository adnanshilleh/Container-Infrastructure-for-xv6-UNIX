...
main(void)
{
	...
	while (getcmd(buf, sizeof(buf)) >= 0) {
		...
		if (fork1() == 0)
		{
			...
			// My naive way of trying to prevent a process from going under root
			// Left here for reference but commented out because it also prevents
			// procs that are not part of containers from getting to root.
			// See: Issue with getting procs to inherit their parents fakeroot
